...
class Flag_FRI_GINConvNet(torch.nn.Module):
    def __init__(self, n_output=1,num_features_xd=78, num_features_xt=25,
                 n_filters=32, embed_dim=128, output_dim=128, dropout=0.2):

        super(Flag_FRI_GINConvNet, self).__init__()
        ...
        # triple convolution
        self.fc_xt = nn.Linear(4096, 2048)
        self.fc_xt2 = nn.Linear(2048, 512)
        self.fc_xt3 = nn.Linear(512, 128)
    ...
    def forward(self, data):
        ...
        # concat
        xc = torch.cat((x, xt), 1)
        xc = torch.cat((xc, flag.unsqueeze(1)), 1)
